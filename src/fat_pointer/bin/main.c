...
loop_array_of_rand_interfaces(
	const size_t n_interfaces, struct rand_interface_i *const rands[n_interfaces], uint64_t *const result)
{
	int status = EXIT_FAILURE;

	const struct rand_interface_i **rand_array = calloc(N_ITER, sizeof rand_array[0]);
	if (!rand_array) {
...
loop_single_rand_interface(
	const struct rand_interface_i *const restrict rand, uint64_t *const result)
{
	int status = EXIT_FAILURE;
	uint64_t result_ = 0;
...
int main(void)
{
	...
	{ // Looping over many of a mix of rand_interface_i implementations.
		...
		struct rand_interface_i *rands[N_IMPLS + N_IMPLS] = {0};
...
test_1_error:
		for (size_t idx = 0; idx < 2 * N_IMPLS; idx++) free(rands[idx]);
...
test_1_xoshiro256ss_rand_create_error:
	...
	{ // Looping over many of a single rand_interface implementation.
		...
		struct rand_interface_i *rands[2 * N_IMPLS] = {0};
...
test_2_xoshiro256ss_rand_create_error:
	...
	{ // Looping with one randomly picked interface.
		...
		struct rand_interface_i *rand = {0};
		if (drand48() < 0.5) {
...
test_3_xoshiro256ss_rand_create_error:
	...
	{ // Looping with one known interface.
		...
		struct rand_interface_i *rand = xoshiro256ss_rand_as_interface(xoshiro256ss_rand);
